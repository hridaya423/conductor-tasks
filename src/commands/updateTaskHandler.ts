...
export async function updateTaskHandler(
  ...
  try {
    ...
    if (priority) {
      switch (priority) {
        case "critical": updates.priority = TaskPriority.CRITICAL; break;
        case "high": updates.priority = TaskPriority.HIGH; break;
        case "medium": updates.priority = TaskPriority.MEDIUM; break;
        case "low": updates.priority = TaskPriority.LOW; break;
        case "backlog": updates.priority = TaskPriority.BACKLOG; break;
      }
    }

    if (status) {
      switch (status) {
        case "backlog": updates.status = TaskStatus.BACKLOG; break;
        case "todo": updates.status = TaskStatus.TODO; break;
        case "in_progress": updates.status = TaskStatus.IN_PROGRESS; break;
        case "review": updates.status = TaskStatus.REVIEW; break;
        case "done": updates.status = TaskStatus.DONE; break;
        case "blocked": updates.status = TaskStatus.BLOCKED; break;
      }
    }
