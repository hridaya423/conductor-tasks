...
export class GeminiProvider {
    ...
    async generate(request) {
        try {
            ...
            errorHandler.handleError(new TaskError(`Gemini API error: ${error instanceof Error ? error.message : String(error)}`, ErrorCategory.LLM, ErrorSeverity.ERROR, { operation: 'gemini-generate' }, error instanceof Error ? error : undefined));
        ...
        while (retryCount <= this.maxRetries) {
            try {
                ...
                if (isRetryable && retryCount < this.maxRetries) {
                    errorHandler.handleError(new TaskError(`Gemini API error (retry ${retryCount + 1}/${this.maxRetries}): ${lastError.message}`, ErrorCategory.LLM, ErrorSeverity.WARNING, { operation: 'gemini-complete', additionalInfo: { retry: retryCount + 1 } }, lastError), true);
                    retryCount++;
                    continue;
                }
                errorHandler.handleError(new TaskError(`Gemini API error (after ${retryCount} retries): ${lastError.message}`, ErrorCategory.LLM, ErrorSeverity.ERROR, { operation: 'gemini-complete', additionalInfo: { maxRetriesExceeded: true } }, lastError));
