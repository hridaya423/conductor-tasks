        ...
        while (retryCount <= this.maxRetries) {
            try {
                if (retryCount > 0) {
                    ...
                    errorHandler.handleError(new TaskError(`Retrying PRD parsing (attempt ${retryCount}/${this.maxRetries})`, ErrorCategory.PARSING, ErrorSeverity.WARNING, { operation: 'extractTasksFromPRD', additionalInfo: { retryCount } }), true);
                }
                ...
                errorHandler.handleError(new TaskError(`Failed to extract tasks from PRD after ${retryCount} retries: ${lastError.message}`, ErrorCategory.PARSING, ErrorSeverity.ERROR, { operation: 'extractTasksFromPRD' }, lastError));
                return [];
            }
        }
        return [];
