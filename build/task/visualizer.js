...
export class TaskVisualizer {
    ...
    displayKanbanBoard(options = {}) {
        const { showPriority = true, showComplexity = true, filterPriority, filterTag, compact = false } = options;
        ...
        tasksByStatus.forEach((statusTasks, status) => {
            tasksByStatus.set(status, statusTasks.sort((a, b) => {
                const priorityOrder = {
        ...
        if (filterPriority || filterTag) {
            output += chalk.italic('\nActive Filters: ');
            if (filterPriority)
                output += chalk.white(`Priority=${filterPriority} `);
            if (filterTag)
                output += chalk.white(`Tag=${filterTag} `);
            output += '\n';
        }
        return output;
    }
    displayDependencyTree(rootTaskId) {
        if (rootTaskId) {
            ...
            if (topLevelTasks.length === 0) {
                ...
                topLevelTasks.forEach((task, index) => {
                    ...
                    if (depTree) {
                        this.printTaskWithDependencies(depTree.task, depTree.dependencies, '', index === topLevelTasks.length - 1);
                    }
    ...
    printTaskWithDependencies(task, dependencies, prefix, isLast) {
        const branch = isLast ? '└── ' : '├── ';
        const newPrefix = prefix + (isLast ? '    ' : '│   ');
        const statusColor = this.getStatusColor(task.status);
        const priorityColor = this.getPriorityColor(task.priority);
        console.log(`${prefix}${branch}${chalk.bold(task.title)} ` +
        ...
        dependencies.forEach((depTask, index) => {
            ...
            this.printTaskWithDependencies(depTask, this.taskManager.getTaskDependencyTree?.(depTask.id)?.dependencies || [], newPrefix, depIsLast);
        });
    }
    formatTaskCompact(task, options) {
    ...
    formatTaskEnhanced(task, options) {
    ...
    getComplexityIndicator(complexity) {
        if (complexity <= 3) {
