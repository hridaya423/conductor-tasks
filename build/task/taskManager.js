import { TaskPriority, TaskStatus, ContextPriority, CodeSymbolType } from '../core/types.js';
...
function detectWorkspaceDirectory() {
...
export class TaskManager {
    constructor(llmManager, contextManager, tasksFilePath) {
        this.tasks = new Map();
        this.tasksFilePath = '';
    ...
    getEnvPriority(key, defaultValue) {
        if (!process.env[key])
            return defaultValue;
    ...
    async initialize(projectName, projectDescription, filePath) {
        ...
        this.contextManager.setProjectContext(`Project: ${projectName}\nDescription: ${projectDescription}`);
    ...
    createDefaultTasksTemplate(projectName, projectDescription) {
    ...
    async createEnhancedInitialTasksWithLLM(projectName, projectDescription) {
        try {
...
`;
            try {
                ...
                for (const taskData of parsedTasks) {
                    try {
                        const taskId = await this.createTask(taskData.title, taskData.description);
                        ...
                        if (task) {
                            ...
                            if (taskData.subtasks && taskData.subtasks.length > 0) {
                                ...
                                for (const subtaskData of taskData.subtasks) {
                                    try {
                                        const subtaskId = await this.createSubtask(taskId, subtaskData.title, subtaskData.description);
                                        if (subtaskId) {
    ...
    parseTasksFromLLMInit(llmOutput) {
        const tasks = [];
        try {
            const taskBlocks = llmOutput.includes('---')
                ? llmOutput.split('---').filter(block => block.trim().length > 0)
                : llmOutput.split(/\nTITLE: |\nTask \d+:/).filter((block, index) => index === 0 ? block.toLowerCase().includes('title:') : block.trim().length > 0);
    ...
    async createTask(title, description, userInput, parentId) {
        ...
        this.tasks.set(taskId, task);
        this.saveTasks();
        this.contextManager.addContext(`Task ${task.id}: ${task.title}\nDescription: ${task.description}\nPriority: ${task.priority}\nComplexity: ${task.complexity}`, this.mapTaskPriorityToContextPriority(task.priority), 'task-manager', ['task', task.id, ...task.tags]);
        return taskId;
    }
    mapTaskPriorityToContextPriority(priority) {
        switch (priority) {
    ...
    async createSubtask(parentId, title, description, userInput) {
        const parentTask = this.tasks.get(parentId);
        if (!parentTask) {
            return undefined;
        }
    ...
    areAllSubtasksComplete(taskId) {
        const subtasks = this.getSubtasks(taskId);
        if (subtasks.length === 0) {
            return true;
        }
        return subtasks.every(subtask => subtask.status === TaskStatus.DONE || this.areAllSubtasksComplete(subtask.id));
    }
    calculateTaskProgress(taskId) {
        ...
        const completedSubtasks = subtasks.filter(subtask => subtask.status === TaskStatus.DONE || this.areAllSubtasksComplete(subtask.id)).length;
    ...
    async enhanceTaskWithLLM(task, userInput) {
...
The rest of the response should be Markdown.`;
                    ...
                    if (metadata) {
                        ...
                        if (metadata.tags && Array.isArray(metadata.tags)) {
                            task.tags = metadata.tags.filter((tag) => typeof tag === 'string');
                        }
                        ...
                        if (metadata.suggestedSubtasks && Array.isArray(metadata.suggestedSubtasks) &&
                            ...
                            for (const subTaskData of metadata.suggestedSubtasks.slice(0, this.config.defaultSubtasks)) {
                                if (subTaskData && typeof subTaskData.title === 'string' && typeof subTaskData.description === 'string') {
                                    const subId = await this.createSubtask(task.id, subTaskData.title, subTaskData.description);
                                    if (subId) {
        ...
        }, ErrorCategory.LLM, 'enhance_task_with_llm', { taskId: task.id });
    }
    setDefaultTaskMetadata(task) {
        task.priority = task.priority || TaskPriority.MEDIUM;
        task.complexity = task.complexity || 5;
        task.tags = task.tags || [];
        task.updatedAt = Date.now();
    ...
    getTasks(options = {}) {
        const { status, priority, tags, sortBy = "priority", sortDirection = "desc" } = options;
...
`;
            ...
            return taskIds;
        }, ErrorCategory.PARSING, 'parse_prd', { additionalInfo: { contentLength: content.length } }, []);
    }
    loadTasks() {
        try {
            ...
            errorHandler.handleError(new TaskError(`Failed to load tasks: ${error instanceof Error ? error.message : String(error)}`, ErrorCategory.FILESYSTEM, ErrorSeverity.ERROR, { operation: 'loadTasks', targetFile: this.tasksFilePath }, error instanceof Error ? error : undefined));
        }
    }
    saveTasks() {
        ...
        try {
            ...
            errorHandler.handleError(new TaskError(`Failed to save tasks: ${error instanceof Error ? error.message : String(error)}`, ErrorCategory.FILESYSTEM, ErrorSeverity.ERROR, { operation: 'saveTasks', targetFile: this.tasksFilePath }, error instanceof Error ? error : undefined));
        }
    }
    tasksToMarkdown() {
    ...
    parseMdToTasks(markdown) {
        try {
                ...
                if (titleMatch) {
                    try {
                        ...
                        const task = {
                            ...
                            priority: priorityMatch ? priorityMatch[1] : TaskPriority.MEDIUM,
                        ...
                        const error = errorHandler.createParsingError(`Error parsing individual task "${titleMatch[1]}" (${taskId})`, {
                            operation: 'parse_task',
                            taskId,
                            additionalInfo: { taskBlock: taskBlock.substring(0, 100) + '...' }
                        }, ErrorSeverity.WARNING, taskParseError instanceof Error ? taskParseError : new Error(String(taskParseError)));
            ...
            const parseError = errorHandler.createParsingError(`Error parsing tasks from markdown`, { operation: 'parse_markdown', targetFile: this.tasksFilePath }, ErrorSeverity.ERROR, error instanceof Error ? error : new Error(String(error)));
            errorHandler.handleError(parseError);
            throw parseError;
        }
    }
    determineStatusFromMarkdown(taskBlock) {
        if (taskBlock.includes('## Backlog'))
            return TaskStatus.BACKLOG;
        if (taskBlock.includes('## To Do'))
            return TaskStatus.TODO;
        if (taskBlock.includes('## In Progress'))
            return TaskStatus.IN_PROGRESS;
        if (taskBlock.includes('## In Review'))
            return TaskStatus.REVIEW;
        if (taskBlock.includes('## Blocked'))
            return TaskStatus.BLOCKED;
        if (taskBlock.includes('## Done'))
    ...
    async getStructuredCodebaseAnalysis(projectName, projectDescription) {
        ...
        const scanDirectory = async (dirPath, isTopLevel = false) => {
            try {
                if (!fs.existsSync(dirPath))
                    return;
...
`;
                try {
                    ...
                    if (llmResponse && llmResponse.text && llmResponse.text.trim().startsWith('[')) {
                        const identifiedRelativePaths = JSON.parse(llmResponse.text.trim());
            ...
            let summaryLines = [`# Codebase Analysis for Project at ${analysisDir}\n`];
    ...
    determineProjectType(stats, importantFiles) {
    ...
    identifyKeyEntryPoints(allFiles, analysisDir, projectType) {
        const entryPoints = [];
    ...
    async analyzeCodebase() {
        ...
        let summaryLines = [`# Codebase Analysis for Project at ${this.workspaceRoot}\n`];
        summaryLines.push(`## I. Overall Stats`);
        ...
        if (structuredSummary.detailedFileAnalyses && structuredSummary.detailedFileAnalyses.length > 0) {
            ...
            structuredSummary.detailedFileAnalyses.forEach(fileAnalysis => {
                ...
                if (fileAnalysis.symbols.length > 0) {
                    ...
                    if (fileAnalysis.symbols.length > 5)
                        summaryLines.push(`  ... and ${fileAnalysis.symbols.length - 5} more.`);
                }
                summaryLines.push('');
            });
        }
    ...
    async performDetailedFileAnalyses(entryPoints, analysisDir) {
        const analyzedFiles = [];
        ...
        for (const relativeFilePath of filesToAnalyze) {
            ...
            if (fs.existsSync(absoluteFilePath) && (absoluteFilePath.endsWith('.ts') || absoluteFilePath.endsWith('.js'))) {
                try {
                    ...
                    const sourceFile = ts.createSourceFile(absoluteFilePath, fileContent, ts.ScriptTarget.Latest, true);
    ...
    extractSymbolsFromFile(sourceFile, relativeFilePath) {
        const symbols = [];
        const imports = [];
        const exports = [];
            ...
            if (ts.isFunctionDeclaration(node) || ts.isMethodDeclaration(node)) {
                ...
                node.members.forEach(member => {
                    ...
                    let memberSymbol = {
                        ...
                        exported: (ts.getCombinedModifierFlags(member) & ts.ModifierFlags.Static) !== 0,
    ...
    _buildDependencyGraph(fileAnalyses, analysisDir) {
        const nodes = [];
        const edges = [];
    ...
    _getCodebaseContextString(analysisSummary) {
        if (!analysisSummary) {
        ...
        if (analysisSummary.languageSummary) {
            ...
            if (topLangs.length > 0)
                contextLines.push(`- **Top Languages:** ${topLangs.join(', ')}`);
        }
        const keyFiles = [];
    ...
    getNodeComment(node, sourceFile) {
        if (!node)
            return undefined;
    ...
    async getTaskTemplate(templateName) {
        const templatesDir = this.getTaskTemplatesDir();
    ...
    _applyTemplateVariables(text, variables) {
    ...
    async createTaskFromTemplateDefinition(templateDefinition, variables, parentId) {
        ...
        const tags = templateDefinition.tags
            ? templateDefinition.tags.map((tag) => this._applyTemplateVariables(tag, variables))
            : [];
        const taskId = await this.createTask(title, description, undefined, parentId);
        const task = this.getTask(taskId);
        if (task) {
            if (templateDefinition.priority)
                task.priority = templateDefinition.priority;
            if (templateDefinition.complexity)
                task.complexity = templateDefinition.complexity;
            task.tags = tags;
    ...
    async createTaskFromTemplate(templateName, variables, parentId) {
    ...
    async generateDiffForChange(filePath, changeDescription, selection) {
...
`;
        try {
            ...
            throw errorHandler.createLLMError(`Failed to generate diff for ${filePath}`, { operation: 'generate_diff', additionalInfo: { filePath, changeDescription } }, ErrorSeverity.ERROR, error instanceof Error ? error : new Error(String(error)));
        }
    }
    isInitialized() {
    ...
    async generateImplementationSteps(taskId) {
        const task = this.tasks.get(taskId);
        if (!task) {
        ...
        const relatedTasks = Array.from(this.tasks.values()).filter(t => t.dependencies.includes(taskId) || task.dependencies.includes(t.id));
        const relatedTasksContext = relatedTasks.length > 0 ?
            'Related tasks:\n' + relatedTasks.map(t => `- ${t.title} (${task.dependencies.includes(t.id) ? 'dependency' : 'dependent'}): ${t.description.substring(0, 100)}...`).join('\n') : '';
        let codebaseContext = 'Codebase context analysis not available or failed.';
        try {
            const structuredAnalysis = await this.getStructuredCodebaseAnalysis();
            codebaseContext = this._getCodebaseContextString(structuredAnalysis);
...
`;
        try {
            ...
            this.addTaskNote(taskId, result.text, 'AI Assistant', 'solution');
            return result.text;
        }
        catch (error) {
            throw errorHandler.createLLMError(`Failed to generate implementation steps`, { operation: 'generate_implementation', taskId }, ErrorSeverity.ERROR, error instanceof Error ? error : new Error(String(error)));
        }
    }
    async expandTask(taskId, expansionPrompt) {
        const task = this.tasks.get(taskId);
        if (!task) {
            throw new Error(`Task with ID ${taskId} not found`);
        }
...
`;
        try {
            ...
            this.addTaskNote(taskId, `# AI Task Expansion Output:\n\n${output}`, 'AI_EXPANDER', 'solution');
            return output;
        }
        catch (error) {
            throw errorHandler.createLLMError(`Failed to expand task`, { operation: 'expand_task', taskId }, ErrorSeverity.ERROR, error instanceof Error ? error : new Error(String(error)));
        }
    }
    async suggestTaskImprovements(taskId) {
        const task = this.tasks.get(taskId);
        if (!task) {
            throw new Error(`Task with ID ${taskId} not found`);
        }
...
`;
        try {
            ...
            this.addTaskNote(taskId, result.text, 'AI Assistant', 'comment');
            return result.text;
        }
        catch (error) {
            throw errorHandler.createLLMError(`Failed to generate task improvement suggestions`, { operation: 'suggest_improvements', taskId }, ErrorSeverity.ERROR, error instanceof Error ? error : new Error(String(error)));
        }
    }
    async mcpInitializeTasks(projectName, projectDescription, absoluteFilePath) {
        try {
            if (!absoluteFilePath) {
...
`;
    ...
    async mcpCreateTask(title, description, additionalContext) {
        try {
            if (!this.initialized) {
